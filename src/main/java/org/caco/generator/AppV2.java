...
/**
 * 一次多个数据生成
 * created by roachjiang 2017/9/14
 */
public class AppV2 {
    public static void main(String[] args) {

        Map<String,Config> beans = BeanFactory.getConfigBean();
        for (  Map.Entry<String,Config> config : beans.entrySet()){
            System.out.println("beanName : " + config.getKey());
            generatorCodeForBean(config.getValue());
        }
    }

    private static void generatorCodeForBean(Config config) {
    ...
    public static void gerenateCode(Table table, String path) {
        ...
        String servicePath = path + "/" + table.getProject() + "/" + table.getModule().replace(".", "/");
        String serviceImplPath = path + "/" + table.getProject()  + "/"+ table.getModule().replace(".", "/");
        String controllerPath = path + "/" + table.getProject() + "/controller/" + table.getModule().replace(".", "/");
        String xmlPath = path + "/" + table.getProject() + "/" + table.getModule().replace(".", "/");
        ...
        CodeUtil.genereateCode("jsp.ftl", table, jspPath + "/ey_" + CodeUtil.toLowerCaseFirstChar(table.getEntityName()) + ".jsp");
